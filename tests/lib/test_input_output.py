...
def test_unicode_input(unicode_filename, verbose=False):
    ...
    output = yaml.unsafe_load(_unicode_open(StringIO.StringIO(data.encode('utf-8')), 'utf-8'))
    ...
    for input in [data, data.encode('utf-8'),
        ...
        output = yaml.unsafe_load(input)
        assert output == value, (output, value)
        output = yaml.unsafe_load(StringIO.StringIO(input))
        assert output == value, (output, value)
...
def test_unicode_input_errors(unicode_filename, verbose=False):
    ...
    for input in [data.encode('latin1', 'ignore'),
        ...
        try:
            yaml.unsafe_load(StringIO.StringIO(input))
        except yaml.YAMLError, exc:
            if verbose:
                print exc
        else:
